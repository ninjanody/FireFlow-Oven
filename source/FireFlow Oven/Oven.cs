...
namespace FireFlow_Oven {
    public partial class Oven : Form {
        ...
        private void Oven_Shown(object sender, EventArgs e) {
            ...
            comboBox_Units.SelectedIndex = Properties.Settings.Default.Units;
        ...
        public void Attempt_Connect_To_Port(string Port_Name) {
            ...
            currentPort.setBaudRate(9600); //9600 bps
            if (currentPort.open()) {
                currentPort.clear();
        ...
        public void Connection_Open() {
            ...
            comboBox_Units.SelectedIndex = GetUnits() == 0 ? 1 : 0;
        }
        ...
        private void Timer_Temperature_Tick(object sender, EventArgs e) {
            if (Com_Port_Connected) {
                ...
                if (FeedBack != string.Empty) {
                    ...
                    Temperature = (int)float.Parse(Temp, CultureInfo.InvariantCulture.NumberFormat);
                }
                ...
                if (FeedBack != string.Empty) {
                    string Temp = FeedBack.Substring(0, FeedBack.Length - 2);
                    PID_Output = (int)float.Parse(Temp, CultureInfo.InvariantCulture.NumberFormat);
                }

                ChartUpdate_Counter++;
                if (ChartUpdate_Counter == TimeBase) {
                    ChartUpdate_Counter = 0;
                    if (comboBox_Units.SelectedIndex == 0)
                        label_Temperature.Text = Temperature + "°C";
                    else if (comboBox_Units.SelectedIndex == 1)
                        label_Temperature.Text = Temperature + "°F";
                    else {
                        Temperature += 274;
                        label_Temperature.Text = Temperature + "°K";
                    }
        ...
        //Comunication
        private byte GetUnits() {
            string FeedBack = SendData("AT+GET_U\r");
            if (FeedBack != string.Empty) {
        ...
        private void checkBox_Clear_History_CheckedChanged(object sender, EventArgs e) {
            ...
            string Data = (checkBox_Clear_History.Checked) ? "AT+PID_Clear\r" : "AT+PID_NoClear\r";
            string FeedBack = SendData(Data);
            if (FeedBack.Contains("AT OK")) {
            }
        }
        ...
        private void comboBox_Units_SelectedIndexChanged(object sender, EventArgs e) {
            OutputChart.ChartAreas[0].AxisY.Title = "Temprature (" + comboBox_Units.SelectedItem + ")";
            int Unit = (comboBox_Units.SelectedIndex == 0) ? 1 : 0; //C or F, if Kelvin read C and convert
            string Data = "AT+SET_U," + Unit + "\r";
            string FeedBack = SendData(Data);
            Properties.Settings.Default.Units = comboBox_Units.SelectedIndex;
            if (FeedBack.Contains("AT OK")) {
                //Do Nothings
            }
        }
        ...
        private void manualToolStripMenuItem_Click(object sender, EventArgs e) {
            System.Diagnostics.Process.Start("https://github.com/FireDeveloper/FireFlow-Oven/blob/master/docs/Fire%20Flow%20Oven%20Manual.pdf");
        }
        private void temperatureControlsToolStripMenuItem_Click(object sender, EventArgs e) {
            ...
            sb.Append(@" \line ");
            sb.Append(@"\b Units: \b0 ");
            sb.Append(@"The units of the set and measurement temperature");
            sb.Append(@"}");
        ...
        private void PIDControlsToolStripMenuItem_Click(object sender, EventArgs e) {
            ...
            sb.Append(@" \line ");
            sb.Append(@"\b Clear History: \b0 ");
            sb.Append(@"After every Gain Update the controller force clear the historical values accosiated with Integral & Derivative gains");
            sb.Append(@" \line ");
